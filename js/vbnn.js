...
VBNN.prototype.removeNode = function(n) {
	...
	for (var e in n.edges) { // TODO var is changed in loop!
		var e2go = n.edges[e];
...
VBNN.prototype.hidePD = function() {
    if (glob3D.pd_objects != []) {
		for (var i=0; i<glob3D.pd_objects.length;i++){
	...
	if (!pd.ready) {
		...
		if (glob3D.pd_objects != []) {
			for (var i=0; i<glob3D.pd_objects.length;i++){
				...
				if (obi.material) {obi.material.dispose();}
				if (obi.texture)  {obi.texture.dispose();}
				if (obi.geometry) {obi.geometry.dispose();}
			}
		...
		*/
		...
		if (pd.ready) {
			...
			case "Circle":
				...
				if (showwire || showbody) { group.add(M3D.addEdgeCylinder(pd.radius,1.0,0.5,0.5,0.5,bodycol,showbody,showwire));
...
VBNN.prototype.update3DBuffer = function() {
	...
	if(glob.showEdges) {
		...
		for (var e in this.edges) {
			...
			var endps = [];
			for (var x in this.edges[e].nodes) {
	...
	if(haveTrace && glob.showTrace) {
		...
		for (var n in this.nodes) {
			...
			if (cn.trace.length > 1) {
				...
				if (glob.straightTrace) {
					...
					for (var j=0;j<cn.trace.length-1;j++){
						//cl(ih);
						//cd(cn.h ist[j]);
						//cd(cn.h ist[j+1]);
						M3D.setTraceEdge(ih, 
		...
		if (prevTraceEdges > ih) {
			// remove unneeded previous edges
			for (var j = ih; j < prevTraceEdges; j++) {
	...
	if (haveTrace && glob.showTrace) {
		...
		for (i in this.nodes) {
			...
			if (strait) {
				...
				for (var j = 0; j < n.trace.length; j++) {
					v = n.trace[j];
					ctx.lineTo(xscale(v.x), yscale(v.y));
				}
			}
